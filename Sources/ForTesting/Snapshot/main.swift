...
do {
    ...
    let serviceGroupConfiguration1 = ServiceGroupConfiguration(services: [consumer1], gracefulShutdownSignals: [.sigterm, .sigint], logger: logger)
    let serviceGroup1 = ServiceGroup(configuration: serviceGroupConfiguration1)
    ...
    try await withThrowingTaskGroup(of: Void.self) { group in
        ...
        group.addTask {
            ...
            // 6 partitions
            var lastOffsetsPerPartition: [KafkaConsumerMessage?] = [nil, nil, nil, nil, nil, nil]
            ...
            for try await record in consumer1.messages {
                ...
                lastOffsetsPerPartition[record.partition.rawValue] = record
                ...
                if tmpCtr >= interval {
                    ...
                    print("[Consumer1] read up to \(record.offset.rawValue) in partition \(record.partition.rawValue), ctr: \(ctr), rate: \(rate) (\(Int(rateMb))KB/s), avgRate: (\(Int(avgRateMb))KB/s), timePassed: \(Int(timeIntervalTotal))sec, totally read: \(sharedCtr.load(ordering: .relaxed))")
            
                    tmpCtr = 0
                    bytes = 0
                    startDate = Date.timeIntervalSinceReferenceDate
                    
                    for offset in lastOffsetsPerPartition {
                        if let offset {
                            try await consumer1.commit(offset) // commit time to time
                        }
                    }
                }

                let messagesReceived = sharedCtr.wrappingIncrementThenLoad(ordering: .relaxed)
                if messagesReceived >= numOfMessages {
                    print("[Consumer1] Messages received: \(messagesReceived)")
                }
                
                if ctr >= numOfMessages {
                    print("[Consumer1] Local ctr reached \(ctr)")
                    break
                }
            }
            let timeIntervalTotal = Date.timeIntervalSinceReferenceDate - totalStartDate
            let avgRateMb = Double(totalBytes) / timeIntervalTotal / 1024
            print("[Consumer2] All read up to ctr: \(ctr), avgRate: (\(Int(avgRateMb))KB/s), timePassed: \(Int(timeIntervalTotal))sec")
        }
        ...
        group.addTask {
            ...
            for try await record in consumer2.messages {
                ...
                if tmpCtr >= interval {
                    ...
                    print("[Consumer2] read up to \(record.offset.rawValue) in partition \(record.partition.rawValue), ctr: \(ctr), rate: \(rate) (\(Int(rateMb))KB/s), avgRate: (\(Int(avgRateMb))KB/s), timePassed: \(Int(timeIntervalTotal))sec, totally read: \(sharedCtr.load(ordering: .relaxed))")
                    ...
                    for offset in lastOffsetsPerPartition {
                        if let offset {
                            try await consumer2.commit(offset) // commit time to time
                        }
                    }
                }
                ...
                let messagesReceived = sharedCtr.wrappingIncrementThenLoad(ordering: .relaxed)
                if messagesReceived >= numOfMessages {
                    print("[Consumer2] Messages received: \(messagesReceived)")
                }
                
                if ctr >= numOfMessages {
                    print("[Consumer2] Local ctr reached \(ctr)")
                    break
                }
            }
            let timeIntervalTotal = Date.timeIntervalSinceReferenceDate - totalStartDate
